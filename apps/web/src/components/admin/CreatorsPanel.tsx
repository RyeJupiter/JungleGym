...
function ApplicationsSection({ initialApplications }: { initialApplications: Application[] }) {
    ...
    <div className="space-y-6">
      ...
      <div>
        <h3 className="text-sm font-semibold text-stone-400 uppercase tracking-wider mb-3">
          Pending <span className="text-stone-300 font-normal normal-case tracking-normal">({pending.length})</span>
        </h3>
        {pending.length === 0 ? (
          ...
          <div className="space-y-3">
            {pending.map((app) => (
              <ApplicationCard key={app.id} app={app} loading={loading} onApprove={approve} onReject={reject} />
            ))}
          </div>
        )}
      </div>
      ...
      {reviewed.length > 0 && (
        <div>
          <h3 className="text-sm font-semibold text-stone-400 uppercase tracking-wider mb-3">
            Reviewed <span className="text-stone-300 font-normal normal-case tracking-normal">({reviewed.length})</span>
          </h3>
          <div className="space-y-3">
            {reviewed.map((app) => (
              <ApplicationCard key={app.id} app={app} loading={loading} onApprove={approve} onReject={reject} />
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <p className="font-bold text-stone-900">{app.profiles?.display_name ?? 'Unknown'}</p>
...
export function CreatorsPanel({ initialCreators, initialApplications }: { initialCreators: UserSearchResult[]; initialApplications: Application[] }) {
  ...
  return (
    <div className="space-y-10">
      ...
      <div>
        ...
        <ApplicationsSection initialApplications={initialApplications} />
      </div>
