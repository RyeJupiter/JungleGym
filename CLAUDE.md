...
### Video purge cron (follow-up from 2026-04-21 soft-delete work)

Creators can delete videos from `/studio/video/[id]/manage`; rows get `deleted_at = now()` + `published = false` and disappear from every public list. Admins can restore within 30 days via `/admin?tab=issues`. After 30 days a job should hard-delete the row, the video file in the `videos` bucket, the thumbnail, and the audio chunks + VTT in the `transcripts` bucket.

Not built yet — the rows just accumulate with `deleted_at` set. Options when we pick this up:
1. **Cloudflare Workers Cron Trigger** on the existing `junglegym` Worker (`[triggers] crons = ["0 4 * * *"]` in wrangler.toml + a handler that runs at that cron). Zero new infra.
2. **Supabase scheduled function** via `pg_cron` extension.

Either way the job is: `select id, video_url, thumbnail_url, transcript_vtt_path from videos where deleted_at < now() - interval '30 days'` → for each, delete storage objects (video/thumbnail/transcripts/audio/* prefix) then `delete from videos where id = ?`.
