...
Current implementation transcodes client-side via ffmpeg.wasm (`lib/transcodeVideo.ts`, shipped 2026-04-22). Works but slow — ~1–3 min on a 45s 4K HEVC clip from an iPhone 17 Pro Max. Agreed on 2026-04-22 to migrate to CF Stream, reusing the existing live-streaming infra (`lib/cloudflare-stream.ts`, `api/stream/provision`, `api/webhooks/cloudflare-stream`).

**Flow to build:**
1. `POST /api/treehouse/intro-video/upload-url` — mints a CF Stream direct-upload URL via `tus` or the one-time-upload endpoint, returns `{ uploadURL, uid }`.
2. Client POSTs the raw file straight to `uploadURL` — bypasses the Worker 100MB body limit; CF Stream accepts up to 30GB, any format.
3. Save `{ uid, status: 'processing' }` into the section's data in `treehouse_config` (JSON blob, no schema change).
4. Poll a new `/api/stream/status/{uid}` endpoint from the editor for ~60s, OR extend `/api/webhooks/cloudflare-stream/route.ts` to handle VOD `video.ready` events (currently scoped to live sessions). Client-side polling is the simpler MVP.
5. View mode: reuse `StreamPlayer` with the Stream `uid` — HLS adaptive bitrate, smooth on mobile.

**Wins:** upload in seconds, no ffmpeg.wasm on the critical path, HLS delivery. **Tradeoff:** 30–60s processing window — show a friendly "Getting your video ready…" placeholder. Billing ~$0.05/hr stored, negligible at intro-video scale.

**Can delete when migrated:** `lib/transcodeVideo.ts`. Keep `lib/ffmpeg.ts` — still used by audio transcription.
