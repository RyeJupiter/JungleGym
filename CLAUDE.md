...
**Shipped 2026-04-23.** Treehouse intro videos now upload directly to CF Stream via Direct Creator Upload. Flow:

1. `POST /api/treehouse/intro-video/upload-url` — auth + creator-only gate, calls `createDirectUpload` in `lib/cloudflare-stream.ts`, returns `{ uploadURL, uid }`.
2. Client POSTs raw file (any format, multipart/form-data) to `uploadURL` via XHR for progress events.
3. `POST /api/treehouse/intro-video/persist { sectionId, streamUid }` — writes the UID into `profiles.treehouse_config` immediately, independent of the editor's batched Save. Solves the "user navigates away before clicking Save and loses the upload" bug. Also deletes the previous CF Stream video for that section (billing cleanup).
4. `GET /api/treehouse/intro-video/status?uid=…` — polled every 4s from the editor until `readyToStream` flips true.
5. Playback: `<iframe src="https://customer-{code}.cloudflarestream.com/{uid}/iframe">`. Legacy `section.data.url` (pre-migration Supabase storage uploads) still renders via `<video>` as a fallback.

**Removed:** `lib/transcodeVideo.ts` (client-side ffmpeg.wasm transcode). `lib/ffmpeg.ts` stays — audio transcription still uses it.
...
| 2026-04-22 | Rye/Claude | Fix silent avatar upload failure in treehouse editor | `handlePhotoChange` only wrote to React state; `handleSave` reads edits from `profileEditsRef.current`. `photo_url` stayed undefined on the ref, so the upload block in `handleSave` was skipped entirely — save appeared to succeed, picture never changed. Mirror `handleFieldChange`'s ref+state pattern. Same bug silently broke photo removal too (also fixed). Regression from `63685f8`, when the ref was introduced to fix a separate onBlur race on text fields and `handlePhotoChange` wasn't updated alongside. Commit `13d46e1`. |
| 2026-04-22 | Rye/Claude | Intro-video upload: accept any format + client-side transcode | TYus reported "upload dialog closes silently" on iPhone. iPhone records `.MOV` (`video/quicktime`), which the `intro-videos` bucket rejects; the `<input accept="video/mp4,video/webm">` filter then silently dropped the file with no error path. Modern 4K HEVC clips also routinely exceed the 100MB bucket cap. Widened `accept="video/*"` and added `lib/transcodeVideo.ts` — re-encodes any input to H.264/AAC MP4 at 720p max with `+faststart` (CRF 26, veryfast preset). Extracted `lib/ffmpeg.ts` so audio transcription and video transcode share a single cached ffmpeg.wasm instance. New `IntroVideoSection` progress UI covers loading-ffmpeg → transcoding (with % bar) → uploading, plus an inline error panel replacing the old `alert()`. Commit `8c73c53`. **Known limit:** transcode is slow — ~1–3 min on laptop-class hardware for a 45s 4K HEVC clip. Follow-up: migrate to Cloudflare Stream (see roadmap § Intro video → Cloudflare Stream). |
| 2026-04-23 | Rye/Claude | Intro-video: migrate to Cloudflare Stream (real fix) | @jupiter tried uploading, left the profile page before the ffmpeg.wasm transcode finished — upload was lost because the treehouse config only persists on Save click, and transcode took minutes. Migrated the whole flow to CF Stream Direct Creator Upload: client calls `/api/treehouse/intro-video/upload-url` for a one-time signed URL, POSTs the raw file straight to CF (any format, any size within CF's cap, no in-browser transcode), then hits `/api/treehouse/intro-video/persist` to write the Stream UID into `profiles.treehouse_config` *immediately* — decoupled from the editor's batched Save so navigating away no longer orphans the upload. Polling `/api/treehouse/intro-video/status` surfaces CF's transcode progress (usually 15–60s). Playback via CF iframe. Legacy `section.data.url` uploads (the Supabase-storage path) still render via `<video>` — no data migration needed. Server-side creator-only gate on both endpoints matches the client-side role gating shipped the same day. CSP `connect-src` extended with `upload.videodelivery.net`. Removed `lib/transcodeVideo.ts`; kept `lib/ffmpeg.ts` (audio transcription). |
| 2026-04-29 | Davis | Creator gift payouts (cron + pull) + Stripe audit infra | Closed the gap in the wallet flow: gift money entering JungleGym's Stripe balance now actually routes to creators. Two settlement modes, both via `stripe.transfers.create` to creators' connected accounts: **Monthly cron** (`POST /api/cron/payout-creator-gifts`, `.github/workflows/payout-creator-gifts.yml` on the 1st at 04:00 UTC) bundles each creator's unsettled gifts ≥ $10 into one Transfer, carves a flat $2 fee. **On-demand pull** (`POST /api/payouts/withdraw`, creator-self-service via Studio "Gifts received" section) — same flow but 5% fee + 7-day rate limit. Both use Stripe idempotency keys so retries never double-pay. **Design decision**: creators' wallet balance and gift earnings are kept in separate buckets — wallet is for gifting other creators; received gifts live in their own studio section sourced directly from `gifts` table. No `gift_received` writes to `wallet_transactions`. **Stripe audit + fraud-proofing**: new `stripe_events` table (PK = Stripe event id) is written by every webhook for built-in idempotency; duplicate deliveries short-circuit. Added webhook cases: `charge.succeeded` (records `balance_transaction` + `charge_id` on the matching `wallet_transactions` row), `transfer.reversed` (unwinds gift settlement, marks `creator_payouts.status = reversed`, admin issue), `payout.paid` (informational), `radar.early_fraud_warning.created` (admin issue, action-now). `wallet_transactions` got `stripe_payment_intent_id` (with unique partial index `uq_wallet_tx_pi` replacing the legacy `description`-keyed one) + `stripe_charge_id` + `stripe_balance_tx_id`. `topup/confirm` now sanity-checks `paymentIntent.amount === walletAmount + fee` against fresh server calc (defense in depth). `gifts` got `settled_at`, `transfer_id`, `settlement_fee`, `settlement_mode`. New `creator_payouts` aggregate table — one row per Stripe Transfer with status driven by webhooks. Migration `00030_payment_audit_and_settlement.sql` includes a backfill from `description='topup:pi_…'` → `stripe_payment_intent_id` before enforcing the unique index. Studio shows lifetime gross + unsettled balance + recent payout history; collapsed entirely if the creator has no gift history yet. Wallet UI in Settings now explicitly says "Gifts you receive are tracked separately under Gifts received" so creators don't expect them to land in the wallet balance. Full plan + Stripe fee table + open questions in `docs/payment-infra.md`. **Reuses existing `CRON_SECRET`** (already a GH + Worker secret from the purge cron). |
